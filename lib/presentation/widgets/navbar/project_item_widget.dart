...
class ProjectItemWidget extends ConsumerWidget {
  final TaskCalendar project;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final isSelected = GoRouterState.of(context).uri.path == '/project/${project.uid}';
    
    // Count remaining tasks for this project
    final taskListAsync = ref.watch(taskListProvider);
    final taskCount = taskListAsync.when(
      data: (tasks) => tasks.where((task) {
        return task.status != 'COMPLETED' && task.sourceCalendarUid == project.uid;
      }).length,
      loading: () => 0,
      error: (_, _) => 0,
    );
    
    final projectWidget = Padding(
      ...
      child: Material(
        ...
        child: InkWell(
          borderRadius: BorderRadius.circular(8),
          onTap: () {
            AppLogger.info('ProjectItem: Navigating to project ${project.uid} (${project.summary})');
    ...
    return Draggable<ProjectDragData>(
      data: ProjectDragData(
        project: project,
        currentDomain: project.flowitDomain,
      ),
      ...
      onDragStarted: () {
        AppLogger.info('ProjectItem: Started dragging project ${project.summary}');
        // Provide haptic feedback
        HapticFeedback.lightImpact();
      },
      onDragEnd: (details) {
        AppLogger.info('ProjectItem: Ended dragging project ${project.summary}');
      },
      child: projectWidget,
    );
  }
  ...
  void _handleMenuAction(BuildContext context, WidgetRef ref, String action) {
    switch (action) {
      case 'move_to_domain':
        _showMoveToDomainDialog(context, ref);
        break;
      case 'archive_project':
        _handleArchiveProject(context, ref);
        break;
      case 'delete_project':
  ...
  void _showMoveToDomainDialog(BuildContext context, WidgetRef ref) {
    showDialog(
      context: context,
  ...
  void _showDeleteConfirmation(BuildContext context, WidgetRef ref) {
    showDialog(
      ...
      builder: (BuildContext dialogContext) => AlertDialog(
        ...
        content: Text(
          'Are you sure you want to delete "${project.summary}"? This action cannot be undone and will remove all associated tasks.',
        ),
        actions: [
          TextButton(
  ...
  void _deleteProject(BuildContext context, WidgetRef ref) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Delete functionality not yet implemented for ${project.summary}'),
        backgroundColor: Theme.of(context).colorScheme.error,
        action: SnackBarAction(
          label: 'Dismiss',
          onPressed: () {},
